...
@section('page-contents')
    ...
    <div class="container-fluid">
        ...
        <div class="head-name d-flex justify-content-between ">
            ...
            <div class="p-1">
                <form action="{{ route('admin.college-search') }}" method="GET"
            ...
            <div class="d-flex">
                ...
                <div class="p-1">
                    <a href="{{ route('admin.college-export') }}"
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            <div class="card-header d-flex justify-content-between py-3">
                <h6 class="my-auto font-weight-bold text-primary">College Data</h6>
                <a href="{{ route('admin.college-data.create') }}" class="btn btn-primary btn-icon-split">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="dataTable" class="table nowrap align-middle" style="width:100%">
                            ...
                            <tr>
                                <th scope="row">
                                    <div class="form-check">
                                        <input class="form-check-input fs-15" type="checkbox" name="checkAll" id="checkAll"
                                            value="option1">
                                    </div>
                                </th>
                                <th>Sl No.</th>
                        ...
                        <tbody>
                            @foreach ($college_datas as $index => $college_data)
                                <tr>
                                    <th scope="row">
                                        <div class="form-check">
                                            <input class="form-check-input fs-15 reg-checkbox" type="checkbox"
                                                name="checkOne[]" value={{ $college_data->id }}>
                                        </div>
                                    </th>
                                    <td>{{ $college_datas->firstItem() + $index }}</td>
                                    <td>{{ $college_data->dise_code }}</td>
                                    <td>{{ $college_data->college_name }}</td>
                                    <td>{{ $college_data->district }}</td>
                                    <td>
                                        <a href="{{ route('admin.college-data.show', $college_data->id) }}"><button
                                                type="button" class="btn btn-sm bg-paid">Details</button></a>

                                        <a href="{{ route('admin.college-data.edit', $college_data->id) }}"><i
                                                class="bg-partial p-2 far fa-edit"></i></a>
                                        <a class="deleteRecord" data-id="form-submit-{{ $college_data->id }}"
                                            data-route="{{ route('admin.college-data.destroy', $college_data->id) }}">
                                            <i class="bg-unpaid p-2 far fa-trash-alt"></i>
                                        </a>
                                        <form method="POST" id="form-submit-{{ $college_data->id }}"
                                            action="{{ route('admin.college-data.destroy', $college_data->id) }}" hidden>
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                    </td>
                    ...
                    {{ $college_datas->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer" style="align-content: center !important">
                    <div>
                        ...
                        <a href="{{ asset('admin/img/college-data.xlsx') }}">
                            <button class="d-none d-sm-inline-block btn btn-sm btn-info shadow-sm">Download </button>
                        </a>
                    </div>
                    <div>
                        <form action="{{ route('admin.college-import') }}" method="POST" enctype="multipart/form-data">
                            @csrf
    ...
    <form id="submit-form" action="{{ route('admin.college-send-bulk-mail') }}" method="post" hidden>
        {{ csrf_field() }}
        <input type="hidden" id="email_template" name="email_template">
        <input type="hidden" id="college_ids" name="college_ids">
    </form>
@endsection
@push('scripts')
    <script>
        ...
        function getSelectedItems() {
            ...
            $('#college_ids').val(JSON.stringify(getCheckedValues));
        }
